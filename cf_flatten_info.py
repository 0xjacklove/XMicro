...
class cf_flatten_info_t:
	def __init__(self,optblock):
		#self.optblock.black_list=[]
		#self.optblock.white_list=[]
		#self.plugin = plugin
	...
	def get_assigned_and_comparison_variables(self, blk):
		...
		if jzc.n_max_jz < 0:
			...
			if not was_white_listed:
				self.optblock.black_list.append(ea)
			return False
		...
		if not was_white_listed:
			if jzc.seen_comparisons[jzc.n_max_jz].should_blacklist():
				self.optblock.black_list.append(ea)
				return False
			self.optblock.white_list.append(ea)
